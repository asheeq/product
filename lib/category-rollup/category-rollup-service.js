...
module.exports = (db) => {
  ...
  function allowDuplication({partOf, madeUpOf, createdBy}) {
    return db.sequelize.query(
      'INSERT INTO product_category_rollup (part_of, made_up_of, created_by, created_at)' + 
      'VALUES(?,?,?,?) ON DUPLICATE KEY UPDATE status = 1, updated_at = (?), updated_by = (?);',
      {
        replacements: 
        [partOf, madeUpOf, createdBy, new Date(), new Date(), createdBy]
      }
    )
      .catch(err => {
        console.log(err);
